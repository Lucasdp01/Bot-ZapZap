...
client.on("message_create", async (message) => {
  if (message.body.toLowerCase() === "agendar" && !message.fromMe) {
    ...
    await client.sendMessage(
      message.from,
      "Está querendo agendar para lembrar algo? Digite um título para o agendamento:"
    );
  } else if (
    msgID &&
    msgID === message.from &&
    ...
    await client.sendMessage(
      message.from,
      "Agora me diga o horário no formato: *HH:MM*."
    );
    ...
    if (hrRegex.test(message.body)) {
      ...
      await client.sendMessage(
        message.from,
        'Agendamento completo. _Para ver sua lista de agendamentos, digite "Ver Agendamentos"_.'
      );
    ...
    message.body.toLowerCase() === "ver agendamentos" &&
    !message.fromMe
  ) {
    ...
    if (agendamentosUsuario && agendamentosUsuario.length > 0) {
      ...
      agendamentosUsuario.forEach((agendamento, index) => {
        resposta += `${index + 1}. *${
          agendamento.titulo
        }* às ${agendamento.horario.toLocaleTimeString()}\n`;
      });
